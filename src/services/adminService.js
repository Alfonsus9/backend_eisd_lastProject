...
async function updateUserStatus(id_user, status, currentUserId) {
    ...
    if (id === currentUserId && status === "nonaktif") throw new Error("CANNOT_DISABLE_SELF");

    const user = await prisma.user.findUnique({ where: { id_user: id } });
    if (!user) throw new Error("NOT_FOUND");

    return prisma.user.update({ where: { id_user: id }, data: { status: status === "aktif" } });
}
...
async function createParkingArea({ name_area, location, kapasitas_total }) {
    if (!name_area || !location || !kapasitas_total) throw new Error("MISSING_FIELDS");
    const kapasitas = Number(kapasitas_total);
    if (Number.isNaN(kapasitas) || kapasitas <= 0) throw new Error("INVALID_KAPASITAS");

    const exists = await prisma.areaParkir.findFirst({ where: { name_area } });
    if (exists) throw new Error("NAME_TAKEN");

    const area = await prisma.areaParkir.create({ data: { name_area, location, kapasitas_total: kapasitas } });
    return area;
}

async function updateParkingArea(id_area, { name_area, location, kapasitas_total }) {
    ...
    if (!name_area && !location && (kapasitas_total === undefined || kapasitas_total === null)) throw new Error("NO_FIELDS");

    if (name_area) {
        const other = await prisma.areaParkir.findFirst({ where: { name_area, NOT: { id_area: id } } });
        if (other) throw new Error("NAME_TAKEN");
    }
    ...
    if (kapasitas_total !== undefined && kapasitas_total !== null) {
        ...
        const activeCount = await prisma.logParkirLog.count({ where: { id_area: id, waktu_keluar: null } });
        if (kapasitas < activeCount) throw new Error("KAPASITAS_TOO_SMALL");
    }

    return prisma.areaParkir.update({ where: { id_area: id }, data: { name_area, location, kapasitas_total } });
}
...
async function deleteParkingArea(id_area) {
    ...
    const activeCount = await prisma.logParkirLog.count({ where: { id_area: id, waktu_keluar: null } });
    if (activeCount > 0) throw new Error("AREA_NOT_EMPTY");
