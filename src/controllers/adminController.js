...
async function createParkingArea(req, res) {
    try {
        ...
        if (!name_area || !location || kapasitas_total === undefined) {
            return res.status(400).json({
                status: false,
                message: "Field name_area, location, dan kapasitas_total wajib diisi",
                data: null,
            });
        }
        ...
        try {
            ...
            if (err.message === "INVALID_KAPASITAS") {
                return res.status(400).json({
                    status: false,
                    message: "kapasitas_total harus berupa angka lebih dari 0",
                    data: null,
                });
            }
            if (err.message === "NAME_TAKEN") {
                return res.status(409).json({
                    status: false,
                    message: "Nama area parkir sudah digunakan",
                    data: null,
                });
            }
            throw err;
...
async function updateParkingArea(req, res) {
    try {
        ...
        try {
            await adminService.updateParkingArea(id, {
                name_area,
                location,
                kapasitas_total,
            });
            ...
            if (err.message === "KAPASITAS_TOO_SMALL") {
                return res.status(400).json({
                    status: false,
