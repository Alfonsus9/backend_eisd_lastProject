...
async function listForSatpam(req, res) {
    try {
        ...
        if (status && !["pending", "diproses", "selesai"].includes(status)) {
            return res.status(400).json({
                status: false,
                message: "Filter status hanya boleh: pending, diproses, atau selesai",
                data: null,
            });
        }
        ...
        const result = await violationReportService.listViolationReports({
            page: pageNum,
            limit: limitNum,
            status,
        });
...
async function updateStatus(req, res) {
    try {
        ...
        try {
            await violationReportService.updateViolationReportStatus(
                id_reports,
                status
            );
        } catch (err) {
...
async function exportReports(req, res) {
    try {
        ...
        let reports;
        try {
            reports = await violationReportService.getReportsForExport({
                start_date,
                end_date,
                status,
            });
        ...
        res.setHeader(
            "Content-Disposition",
            "attachment; filename=violation_reports.pdf"
        );
